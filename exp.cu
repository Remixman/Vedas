...
int main(int argc, char *argv[]) {
  ...
  int start = 100;
  vector<TYPEID> sparse_subject, sparse_object;
  ...
  for (int i = 0; i < 100; i++) {
    sparse_subject.push_back( randomId(start, nn * 4) );
    sparse_object.push_back( randomId(start, nn * 4) );
  }
  
  cout << "DENSE SIZE : " << outer_subject.size() << "\n";
  cout << "SPARSE SIZE : " << sparse_subject.size() << "\n";
#if 0
  // DENSE CPU HASH JOIN
  cpuHashJoin(outer_subject, outer_object, inner_subject, inner_object); // warm up
  ss = 0;
  for (int t = 0; t < 3; t++) {
    chrono::steady_clock::time_point hbegin = chrono::steady_clock::now();
    cpuHashJoin(outer_subject, outer_object, inner_subject, inner_object);
    chrono::steady_clock::time_point hend= chrono::steady_clock::now();
    ss += chrono::duration_cast<chrono::microseconds>(hend - hbegin).count();
  }
  cout << "CPU Hash Join  : " << ss / 3.0 / 1000000.0 << " Sec.\n";

  // DENSE GPU HASH JOIN
  sortMergeJoin(context, outer_subject, outer_object, inner_subject, inner_object);
  ss = 0;
  for (int t = 0; t < 3; t++) {
    chrono::steady_clock::time_point smjbegin = chrono::steady_clock::now();
    sortMergeJoin(context, outer_subject, outer_object, inner_subject, inner_object);
    chrono::steady_clock::time_point smjend = chrono::steady_clock::now();
    ss += chrono::duration_cast<chrono::microseconds>(smjend - smjbegin).count();
  }
  cout << "GPU Sort Merge Join  : " << ss / 3.0 / 1000000.0 << " Sec.\n";
#endif
  // SPARSE CPU HASH JOIN
  cpuHashJoin(sparse_subject, sparse_object, inner_subject, inner_object); // warm up
  ss = 0;
  for (int t = 0; t < 3; t++) {
    chrono::steady_clock::time_point schbegin = chrono::steady_clock::now();
    cpuHashJoin(sparse_subject, sparse_object, inner_subject, inner_object);
    chrono::steady_clock::time_point schend= chrono::steady_clock::now();
    ss += chrono::duration_cast<chrono::microseconds>(schend - schbegin).count();
  }
  cout << "CPU Sparse Hash Join  : " << ss / 3.0 / 1000000.0 << " Sec.\n";

  // SPARSE GPU HASH JOIN
  hashJoin(context, sparse_subject, sparse_object, inner_subject, inner_object); // warm up
  ss = 0;
  for (int t = 0; t < 3; t++) {
    chrono::steady_clock::time_point ghjbegin = chrono::steady_clock::now();
    hashJoin(context, sparse_subject, sparse_object, inner_subject, inner_object);
    chrono::steady_clock::time_point ghjend = chrono::steady_clock::now(); 
    ss += chrono::duration_cast<chrono::microseconds>(ghjend - ghjbegin).count();
  }
  cout << "GPU Sparse Hash Join  : " << ss / 3.0 / 1000000.0 << " Sec.\n";
  
  // SPARSE GPU HASH JOIN
  sortMergeJoin(context, outer_subject, outer_object, inner_subject, inner_object);
  ss = 0;
  for (int t = 0; t < 3; t++) {
    chrono::steady_clock::time_point ssmjbegin = chrono::steady_clock::now();
    sortMergeJoin(context, sparse_subject, sparse_object, inner_subject, inner_object);
    chrono::steady_clock::time_point ssmjend = chrono::steady_clock::now();
    ss += chrono::duration_cast<chrono::microseconds>(ssmjend - ssmjbegin).count();
  }
  cout << "GPU Sparse Sort Merge Join  : " << ss / 3.0 / 1000000.0 << " Sec.\n";
